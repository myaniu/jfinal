...
public class ShiroPlugin implements IPlugin {
	...
	private static final Class<? extends Annotation>[] AUTHZ_ANNOTATION_CLASSES = new Class[] {
			RequiresIps.class, RequiresPermissions.class, RequiresRoles.class, RequiresUser.class,
	...
	private void scanAnnotation(List<AuthzHandler> authzArray,
		...
		for (Annotation a : annotations) {
			if (a instanceof RequiresIps) {
				authzArray.set(0, new IpAuthzHandler(a));
			} else if (a instanceof RequiresRoles) {
				authzArray.set(1, new RoleAuthzHandler(a));
				...
				authzArray.set(3, AuthenticatedAuthzHandler.me());
			} else if (a instanceof RequiresUser) {
				authzArray.set(4, UserAuthzHandler.me());
			}  else if (a instanceof RequiresGuest) {
				authzArray.set(5, GuestAuthzHandler.me());
			}
		}
	}
